...
| File | Purpose |
|------|---------|
| `kolla/common/sources.py` | `SOURCES` dict with URLs containing variable placeholders |
| `kolla/common/config.py` | `TARBALLS_BASE` constant and `tarballs-base` config option |
| `kolla/image/kolla_worker.py` | Substitutes variables and passes sources to build tasks |
| `kolla/image/tasks.py` | `process_source()` method performs actual downloads |

### How Source Downloads Work

1. **Source Definition** (`sources.py`):
   ```python
   SOURCES = {
       'nova-base': {
           'type': 'url',
...
2. **Variable Substitution** (`kolla_worker.py`):
   - `$tarballs_base` → `https://tarballs.opendev.org` (from config)
   - `${openstack_branch}` → e.g., `master` or `stable-2024.2`

3. **Download Execution** (`tasks.py:process_source()`):
   - **URL type**: `requests.get()` to download tarball, verify SHA256 if specified
   - **Git type**: `git.Git().clone()` with optional ref checkout
   - **Local type**: Copy from local filesystem

### Example Flow

For `kolla-build nova --openstack-release master`:

1. `SOURCES['nova-base']` provides URL template
2. Variables substituted: `https://tarballs.opendev.org/openstack/nova/nova-master.tar.gz`
3. `process_source()` downloads and extracts to build context
4. Dockerfile `ADD` instruction includes the source
