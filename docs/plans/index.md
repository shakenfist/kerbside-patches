...
| 2026-09-24 | [Non-OpenStack upstream series (qemu, Linux)](upstream-series.md) | Keep patch series against upstreams outside OpenStack, starting with kerbside's SPICE qemu damage-path series (carried downstream) and a DRM `ignore_damage_clips` kernel fix (to submit), under an `upstream/` tree the OpenStack tooling cannot see, with their measurement rigs, rebase checks and a mail submission path | Not started |
